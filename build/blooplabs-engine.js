...
var _Edible = __webpack_require__(2);

var _Edible2 = _interopRequireDefault(_Edible);

var _Brain = __webpack_require__(3);
...
(function (window) {
    window.BloopLabs = { Edible: _Edible2.default, Brain: _Brain2.default };
})(window);
...
var Edible = function () {
    function Edible(_ref) {
        var _ref$isActive = _ref.isActive,
            isActive = _ref$isActive === undefined ? true : _ref$isActive,
    ...
    _createClass(Edible, [{
        key: "render",
        value: function render(entities, renderer) {
...
var convnetjs = __webpack_require__(4);
...
var Brain = function () {
    function Brain(_ref) {
        var _ref$inputs = _ref.inputs,
            ...
            outputs = _ref$outputs === undefined ? [] : _ref$outputs,
            _ref$midLayerNodes = _ref.midLayerNodes,
            midLayerNodes = _ref$midLayerNodes === undefined ? 4 : _ref$midLayerNodes,
        ...
        this.network = ENCOG.BasicNetwork.create([ENCOG.BasicLayer.create(ENCOG.ActivationTANH.create(), this.inputs.length, 1), ENCOG.BasicLayer.create(ENCOG.ActivationTANH.create(), this.midLayerNodes, 1), ENCOG.BasicLayer.create(ENCOG.ActivationTANH.create(), this.outputs.length, 0)]);
        this.net = new convnetjs.Net();
        this.net.makeLayers([{ type: 'input', out_sx: 1, out_sy: 1, out_depth: this.inputs.length }, { type: 'fc', num_neurons: this.midLayerNodes, activation: 'tanh' }, { type: 'regression', num_neurons: this.outputs.length }]);
    }

    _createClass(Brain, [{
        key: 'compute',
        value: function compute(env, agent, entities) {
            var inputValues = this.inputs.map(function (i) {
                return Inputs[i].process(env, agent, entities);
            });
            var inputVol = new convnetjs.Vol(inputValues);
            var outputVol = this.net.forward(inputVol);
            this.outputs.map(function (o, index) {
                return Outputs[o].process(outputVol.w[index], agent);
            });
        }
    }, {
        key: 'extract',
        value: function extract() {
            return {
                inputs: [].concat(_toConsumableArray(this.inputs)),
                outputs: [].concat(_toConsumableArray(this.outputs)),
                midLayerNodes: this.midLayerNodes,
                weights: [].concat(_toConsumableArray(this.network.weights))
            };
        }
    }, {
        key: 'clone',
        ...
        key: 'mate',
        value: function mate(partnerBrain) {
            var childOptions = Brain.crossover(this.network.weights, partnerBrain.network.weights);
            var childWeights = childOptions[Util.random(1) < 0.5 ? 1 : 0];
            var childBrain = this.clone();
        ...
        value: function mutate() {
            var data = this.network.weights;
            var swap1 = Math.floor(Util.random() * data.length);
            var swap2 = Math.floor(Util.random() * data.length);
        ...
        value: function crossover(father, mother) {
            ...
            var cutLength = Math.floor(len / 3);
            var cutA = cutLength;
            var cutB = cutA + cutLength;
            if (cutA > cutB) {
                var tmp = cutB;
                cutB = cutA;
                cutA = tmp;
            }
            var child1 = [].concat(_toConsumableArray(father.slice(0, cutA)), _toConsumableArray(mother.slice(cutA, cutB)), _toConsumableArray(father.slice(cutB, len)));
            var child2 = [].concat(_toConsumableArray(mother.slice(0, cutA)), _toConsumableArray(father.slice(cutA, cutB)), _toConsumableArray(mother.slice(cutB, len)));
